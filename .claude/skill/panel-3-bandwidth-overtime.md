...
## Major Update (2025-12-26): Split into Separate Download/Upload Panels

**Old Design:** Single panel with download (positive) + upload (negative) = 2N legend entries
**New Design:** Separate panels - Panel 3 (Download), Panel 11 (Upload) = N entries each

**Why Changed:**

- ❌ Old: 2 legend entries per device = 2N total (with top_n=10 → 20 legend entries)
- ✅ New: 1 legend entry per device per panel (cleaner, easier to read)
- ✅ Same devices in both panels (consistent scoring algorithm)

## Critical Features (2025-12-26)

### Security-Focused Device Selection Algorithm

**Scoring Formula:**

```
score = max(download_bytes, upload_bytes) + upload_bonus
where upload_bonus = 999999999999 if upload > 10 Mbps, else 0
```

**What This Means:**

- Top N devices selected by whichever is higher: download or upload
- **Devices uploading > 10 Mbps ALWAYS appear** (huge bonus score)
- Catches exfiltration: Device with 100 MB download + 50 MB upload → appears
- Catches data theft: Device with 10 MB download + 25 MB upload → appears (upload bonus)

### Type Conflict Fix

Added `float(v: r._value)` conversion:

```flux
|> map(fn: (r) => ({ r with _value: float(v: r._value) * 8.0 / ... }))
```

### Multiple Device Display Fix

Added `group()` before `findColumn()` to ungroup scoring results:
...
Without `group()`, findColumn only gets device from first table (single device bug).
